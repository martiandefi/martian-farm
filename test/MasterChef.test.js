...
contract('MasterChef', ([alice, bob, carol, referrer, treasury, dev, fee, owner]) => {
    beforeEach(async () => {
        ...
        this.referral = await MartianReferral.new({ from: owner });
        this.chef = await MasterChef.new(this.martian.address, '100', '1000', { from: owner });

        await this.martian.transferOwnership(this.chef.address, { from: owner });
        await this.referral.updateOperator(this.chef.address, true, { from: owner });
        await this.chef.setMartianReferral(this.referral.address, { from: owner });
